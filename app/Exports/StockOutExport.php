...
class StockOutExport implements FromCollection, WithHeadings, WithStyles, ShouldAutoSize, WithColumnWidths
{
    /**
     * Export stock-out data with related inventory items.
     */
    public function collection()
    {
        return StockOut::with('inventoryItems')->get()->map(function ($stockOut) {
            return [
                'id' => $stockOut->id,
                'date' => $stockOut->created_at->format('Y-m-d'),
                'notes' => $stockOut->notes,
                'items' => $stockOut->inventoryItems->map(function ($item) {
